...
- **左侧分类侧边栏（按三阶段分组）+ 右侧卡片**：阶段1 角色三视图/场景、阶段2 分镜机位、阶段3 视频片段/语音配音；侧边栏底部「设置（配置）」「关闭服务」。
...
**应用内交互**：设置抽屉里显示工作目录路径（带「打开」），「📁 目录」可从工作目录选参考图；
页内每张图片都支持**点击放大预览**与**下载**（参考图缩略图同样可点击预览），✕ 直接删除参考图。
