...
| 阶段 | 任务 | 要点 |
|------|------|------|
| 1 设定 | `characters` + `scenes` | 角色单张三视图合图（勿默认 `--views`）；`gender`/`build`/`style_ref`；场景空镜无人物 |
...
用 brief 对齐需求。参考图**必须对应正确角色**（逐张确认，或让用户在角色卡片上传）。plan 只放 `characters` + `scenes`。可先备 `shots`/`clips` 草稿预览全貌，**生成仍按阶段门控**。
